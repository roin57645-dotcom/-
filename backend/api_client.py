...
def get_system_prompt(persona_profile: str, self_profile: str, history_sessions: list = None, analysis_time: str = "", user_notes: str = "") -> str:
    memory_block = (
        f"## 已知历史记忆与人设背景\n"
        f"### 对方画像\n"
    ...
    time_block = ""
    if analysis_time:
        time_block = f"## 时间上下文\n该截图的分析时间为：{analysis_time}（北京时间）。\n\n"

    # User notes
    notes_block = ""
    if user_notes:
        notes_block = f"## 分析师现场附注（最高优先级参考）\n以下是用户本人对该对话的实时判断与补充背景，你必须将这些信息作为分析的最高优先级参考：\n---\n{user_notes}\n---\n\n"

    return f"你是一个对话动力学分析专家。用户会给你一张聊天截图，请严格按以下规则分析。\n\n{knowledge_block}{time_block}{notes_block}{memory_block}{history_block}{_PROTOCOL}"
...
async def stream_analyze(image_data_url: str, persona_profile: str, self_profile: str, history_sessions: list = None, analysis_time: str = "", user_notes: str = ""):
    client = AsyncOpenAI(api_key=API_KEY, base_url=API_BASE_URL)
    stream = await client.chat.completions.create(
        model=MODEL_NAME,
        messages=[
            {"role": "system", "content": get_system_prompt(persona_profile, self_profile, history_sessions, analysis_time, user_notes)},
            {
                "role": "user",
                "content": [
